...
public class PercentToPoint : MonoBehaviour
{
    ...
    public static int WayPointIndex(float percent)
    {
        //Doe dit eerst voordat je PercentToPath doet zodat de enemy naar de goeie waypoint kan kijken
        float newValue = EnemyPathMaking.t_Points[0].Length * (percent / 100); //Getal word WaypointIndex + % Distance tussen waypoints. Bijv: 8.32
        int waypointIndex = Mathf.FloorToInt(newValue); //WaypointIndex in vorige voorbeeld is dan 8
        return waypointIndex;
    }
    ...
    public static Vector3 PercentToPath(float percent, Quaternion rotation)
    {
        float newValue = EnemyPathMaking.t_Points[0].Length * (percent / 100); //WaypointIndex + % Tussen points
        int waypointIndex = Mathf.FloorToInt(newValue); //Haalt WaypointIndex uit volledig nummer

        float distanceInWaypoint = (newValue - waypointIndex) * EnemyPathMaking.distancePoints[0][waypointIndex - 1]; //% terugzetten naar distance
        Vector3 tDirection = rotation * Vector3.forward; //Waar kijkt de enemy naar

        Vector3 spawnPoint = EnemyPathMaking.t_Points[0][waypointIndex - 1].position + tDirection * distanceInWaypoint; //OMG (Ga het aantal distance naar voren + de startpoint van de huidige waypoint)
        return spawnPoint;
    }
}
